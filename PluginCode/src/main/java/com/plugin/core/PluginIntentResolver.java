...
public class PluginIntentResolver {
    ...
    public static final String SERVICE_ACTION_IN_PLUGIN = "_SERVICE_START_ACTION_IN_PLUGIN_";
    ...
    /* package */
    static void resolveService(Intent service) {
        String className = PluginLoader.matchPlugin(service);
        if (className != null) {
            String serviceName = PluginStubBinding.bindStubService(className);
            if (serviceName != null) {
                service.setComponent(
                        new ComponentName(PluginLoader.getApplicatoin(), serviceName));
            }
            service.setAction(className + SERVICE_ACTION_IN_PLUGIN + (service.getAction() == null ? "" : service.getAction()));
        }
    }

    /* package */
    static void resolveStopService(final Intent service) {
        String className = PluginLoader.matchPlugin(service);
        if (className != null) {
            String serviceName = PluginStubBinding.unBindStubService(className);
            if (serviceName != null) {
                service.setComponent(
                        new ComponentName(PluginLoader.getApplicatoin(), serviceName));
            }
    ...
    /* package */
    static Intent resolveReceiver(final Intent intent) {
    ...
    /* package */
    static Class hackReceiverForClassLoader(Object msgObj) {
    ...
    /* package */
    static String hackServiceName(Object msgObj) {
