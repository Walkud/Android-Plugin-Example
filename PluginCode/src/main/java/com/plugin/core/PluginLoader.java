...
public class PluginLoader {
    ...
    private static final boolean NEED_VERIFY_CERT = true;//验证apk签名标记
    private static final int SUCCESS = 0;//安装插件成功标记
    private static final int SRC_FILE_NOT_FOUND = 1;//安装文件未找到
    private static final int COPY_FILE_FAIL = 2;//复制文件失败
    private static final int SIGNATURES_INVALIDATE = 3;//签名无效
    private static final int VERIFY_SIGNATURES_FAIL = 4;//验证签名失败
    private static final int PARSE_MANIFEST_FAIL = 5;//解析Manifest文件失败
    private static final int FAIL_BECAUSE_HAS_LOADED = 6;//插件已加载，安装失败
    private static final int INSTALL_FAIL = 7;//安装插件失败

    private static Application sApplication;

    private static boolean isLoaderInited = false;//是否已经初始化创建列表
     ...
     * @return
     */
    public static boolean isInstallPluginById(String pluginId) {
        return pluginManager.isInstallPluginById(pluginId);
    }
     ...
     * @return
     */
    public static String getPluginVersionById(String pluginId) {
        PluginDescriptor pluginDescriptor = getPluginDescriptorByPluginId(pluginId);
        if (pluginDescriptor != null) {
